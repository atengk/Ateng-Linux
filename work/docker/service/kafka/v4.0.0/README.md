...
```
docker tag bitnami/kafka:4.0.0 registry.lingo.local/bitnami/kafka:4.0.0
docker push registry.lingo.local/bitnami/kafka:4.0.0
```

**保存镜像**

```
docker save registry.lingo.local/bitnami/kafka:4.0.0 | gzip -c > image-kafka_4.0.0.tar.gz
```
...
advertised.listeners=CLIENT://:9092,INTERNAL://:9094,EXTERNAL://192.168.1.12:20004
...
docker run -d --name ateng-kafka \
  ...
  -e TZ=Asia/Shanghai \
  registry.lingo.local/bitnami/kafka:4.0.0
```
...
```
kafka-console-producer.sh --bootstrap-server 192.168.1.12:20004 --topic test
```

消费消息

```
kafka-console-consumer.sh --bootstrap-server 192.168.1.12:20004 --topic test --from-beginning
```
