...
```
docker tag nacos/nacos-server:v3.2.0 registry.lingo.local/service/nacos-server:v3.2.0
docker push registry.lingo.local/service/nacos-server:v3.2.0
```

**保存镜像**

```
docker save registry.lingo.local/service/nacos-server:v3.2.0 | gzip -c > image-nacos-server_v3.2.0.tar.gz
```
...
docker run -d --name ateng-nacos --user 1001 \
  ...
  -e JVM_XMX=2g \
  -e TZ=Asia/Shanghai \
  registry.lingo.local/service/nacos-server:v3.2.0
```
...
![image-20260407092229660](./assets/image-20260407092229660.png)

Nacos 首页

![image-20260407092509414](./assets/image-20260407092509414.png)
