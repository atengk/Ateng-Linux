...
```
docker pull bitnami/redis:8.2.0
```

**推送到仓库**

```
docker tag bitnami/redis:8.2.0 registry.lingo.local/bitnami/redis:8.2.0
docker push registry.lingo.local/bitnami/redis:8.2.0
```

**保存镜像**

```
docker save registry.lingo.local/bitnami/redis:8.2.0 | gzip -c > image-redis_8.2.0.tar.gz
```
...
docker run -d --name ateng-redis \
  ...
  -e TZ=Asia/Shanghai \
  registry.lingo.local/bitnami/redis:8.2.0
```
