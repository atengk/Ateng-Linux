...
```
docker pull bitnami/redis:8.0.3
```
...
docker push registry.lingo.local/bitnami/redis:8.0.3
```

**保存镜像**

```
docker save registry.lingo.local/bitnami/redis:8.0.3 | gzip -c > image-redis_8.0.3.tar.gz
```
...
docker run -d --name ateng-redis \
  ...
  -e TZ=Asia/Shanghai \
  registry.lingo.local/bitnami/redis:8.0.3
```
