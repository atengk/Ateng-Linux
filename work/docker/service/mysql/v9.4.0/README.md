...
```
docker pull bitnami/mysql:9.4.0
```

**推送到仓库**

```
docker tag bitnami/mysql:9.4.0 registry.lingo.local/bitnami/mysql:9.4.0
docker push registry.lingo.local/bitnami/mysql:9.4.0
```

**保存镜像**

```
docker save registry.lingo.local/bitnami/mysql:9.4.0 | gzip -c > image-mysql_9.4.0.tar.gz
```
...
docker run -d --name ateng-mysql \
  ...
  -e TZ=Asia/Shanghai \
  registry.lingo.local/bitnami/mysql:9.4.0
```
