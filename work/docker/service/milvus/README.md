...
```
docker tag milvusdb/milvus:v2.6.6 registry.lingo.local/service/milvus:2.6.6
docker push registry.lingo.local/service/milvus:2.6.6
```

**保存镜像**

```
docker save registry.lingo.local/service/milvus:2.6.6 | gzip -c > image-milvus_2.6.6.tar.gz
```
...
docker run -d --name ateng-milvus --user=1001 \
  ...
  -e TZ=Asia/Shanghai \
  registry.lingo.local/service/milvus:2.6.6 \
